...
def getCookies():
    ...
    st_sp = currentDate.strftime("%Y-%m-%d") # 2025-05-25
    st_psi = currentDate.strftime("%Y%m%d") # 20251017
    cookies_str = f'qgqp_b_id=90672b8a757593658f7f51013d87539f; st_nvi=oiUBtiJhY6yrkfw2NjDSe98c9; nid=06c1236e9e1b1460187fcb22446deb5f; nid_create_time=1758614485038; gvi=zEYmGUb7o5NU5DonqyRPya975; gvi_create_time=1758614485038; st_si=85913851905698; fullscreengg=1; fullscreengg2=1; st_asi=delete; st_pvi=12807136001264; st_sp={st_sp}%2001%3A02%3A54; st_inirUrl=https%3A%2F%2Fquote.eastmoney.com%2Fconcept%2Fsh601288.html; st_sn=14; st_psi={st_psi}043509547-113200354966-4455534280'
    cookies_dict = {}
...
def req(url: str, methon: str = 'GET', data: dict = {}):
    session = requests.Session()  # 使用Session对象来管理cookies
    for key, value in cookies_dict.items():
        session.cookies.set(key, value)  # 手动设置cookie
    headers = {
        # 'Host': '101.226.30.206:443',
        ...
        # 'X-Forwarded-For': '14.103.191.91:443'
        # 'Remote-Address': '14.103.191.91:443'
    }
    # proxies = {
    #     'http': '43.144.251.121:443',
    #     'https': '43.144.251.121:443',
    # }
    if methon == 'POST':
...
def doSyncFund(code: str, name: str, type: str, pageNum: str):
    ...
    url = f'https://push2.eastmoney.com/api/qt/stock/get?invt=2&fltt=1&cb=jQuery351013051361154176822_{getTimestamp()}&fields=f135%2Cf136%2Cf137%2Cf138%2Cf139%2Cf140%2Cf141%2Cf142%2Cf143%2Cf144%2Cf145%2Cf146%2Cf147%2Cf148%2Cf149&secid={type}.{code}&ut=fa5fd1943c7b386f172d6893dbfba10b&wbp2u=%7C0%7C0%7C0%7Cweb&dect=1&_={getTimestamp()}'
    try:
        resp = req(url, 'GET')
...
def doSyncDetails(code: str, name: str, type: str, date: str, pageNum: str):
    ...
    url = f'https://push2.eastmoney.com/api/qt/stock/details/get?fields1=f1,f2,f3,f4&fields2=f51,f52,f53,f54,f55&fltt=2&cb=jQuery351013051361154176822_{getTimestamp()}&pos=0&secid={type}.{code}&ut=fa5fd1943c7b386f172d6893dbfba10b&wbp2u=%7C0%7C0%7C0%7Cweb&_={getTimestamp()}'
    try:
        resp = req(url, 'GET')
        if resp.status_code == 200:
            resp_text = re.sub(r"^.+\(", "", resp.text)
...
def doSyncByLastDay(page = 1, currentDate: str = datetime.now().strftime("%Y-%m-%d"), endPage = None):
    ...
    with open(os.path.join(os.getcwd(), 'data', 'last_stock.sql'), 'w', encoding='utf-8') as file:
        ...
        while (True):
            ...
            url = f'https://push2.eastmoney.com/api/qt/clist/get?np=1&fltt=1&invt=2&cb=jQuery37108866059046083457_{getTimestamp()}&fs=m%3A0%2Bt%3A6%2Cm%3A0%2Bt%3A80%2Cm%3A1%2Bt%3A2%2Cm%3A1%2Bt%3A23%2Cm%3A0%2Bt%3A81%2Bs%3A2048&fields=f12%2Cf13%2Cf14%2Cf1%2Cf2%2Cf4%2Cf3%2Cf152%2Cf5%2Cf6%2Cf7%2Cf15%2Cf18%2Cf16%2Cf17%2Cf10%2Cf8%2Cf9%2Cf23&fid=f3&pn={pageNum}&pz={pageSize}&po=1&dect=1&ut=fa5fd1943c7b386f172d6893dbfba10b&wbp2u=%7C0%7C0%7C0%7Cweb&_={getTimestamp()}'
            try:
                response = req(url, 'GET')
                if response.status_code == 200:
                ...
                print(f"获取第 {pageNum} 页 HTTP错误发生: {url}")
