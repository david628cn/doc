...
export const closestTableView = (dom: any) => {
    return closest(dom, dom => dom.classList && dom.classList.contains(`${CLASSNAME}-table-view`));
}
...
export const findTable = ($pos: any) => {
    return findParentNodeClosestToPos($pos, n => n.type.spec.tableRole === 'table');
}

export const findCell = ($pos: any) => {
    return findParentNodeClosestToPos($pos, n => n.type.spec.tableRole === 'cell' || n.type.spec.tableRole === 'header_cell');
}
