...
const startResizing = (e: MouseEvent, view: EditorView, parentPos: number, index: number) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
        ...
        parentNode.forEach((child, _, i) => {
            if (i === index) activeRatios.push(finalLeftRatio);
            else if (i === index + 1) activeRatios.push(finalRightRatio);
            else activeRatios.push(child.attrs.ratio);
        });
    ...
    const onMouseUp = () => {
        ...
        for (let i = 0; i < index; i++) leftPos += parentNode.child(i).nodeSize;
        ...
        state: {
            ...
            apply(tr, value) {
                ...
                if (tr.docChanged && value.resizingPos !== null) {
                    return { ...value, resizingPos: tr.mapping.map(value.resizingPos) };
                }
                return value;
            }
        },
                    ...
                    if (node.type.name === 'columns') {
                        ...
                        for (let i = 0; i < node.childCount; i++) {
                            ...
                            if (i < node.childCount - 1) {
                                ...
                                decos.push(
                                    Decoration.widget(handlePos, (view) => {
