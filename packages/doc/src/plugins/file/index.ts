...
export const filePluginKey = new PluginKey('file');
...
export const file = ({
    ...
    const plugin: Plugin = new Plugin({
                ...
                update(view: EditorView) {
                    ...
                    const params: any = {
                        ...
                        command: (url: string) => {
                            ...
                            const filePluginState = filePluginKey.getState(state);
                            const currentPos = filePluginState?.pos;
                            ...
                            // 2. 執行替換/插入
                            const node = state.schema.nodes.file.create({ src: url });
                            const tr = state.tr
                                .replaceWith(currentPos, currentPos, node) // 在精確位置插入
                                .setMeta('file', false);                  // 關閉插件狀態（隱藏模擬光標和彈窗）
        ...
        props: {
            nodeViews: {
                file: (node: Node, view: EditorView, getPos: () => number) => new FileView(node, view, getPos)
            } as any,
            handleKeyDown(view: EditorView, event: any) {
                const state = filePluginKey.getState(view.state);
                if (typeof state.pos !== 'number') {
                    return false; // 我不活跃，放行给后面的插件
                }
                if (event.key === 'Escape' || event.key === 'Esc') {
                    view.dispatch(view.state.tr.setMeta('file', false));
                    return true;
                }
                return false;
            },
            decorations(state) {
                ...
                cursor.className = `${CLASSNAME}-file-cursor`;
                return DecorationSet.create(state.doc, [
                    Decoration.widget(pos, cursor, { key: 'file-cursor', side: -1 })
                ]);
            }
        }
    });
