...
export class AudioView {
    ...
    constructor(node: Node, view: EditorView, getPos: () => number) {
        ...
        this.dom.className = `${CLASSNAME}-audio-view`;
        ...
        this.content.className = `${CLASSNAME}-audio-view-content`;
        ...
        // 禁止原生拖拽，防止干扰缩放
        // this.audio.draggable = false;
        // this.audio.contentEditable = 'false';
        this.updateStatus();

        this.ctrolPanel = document.createElement('div');
        this.ctrolPanel.className = `${CLASSNAME}-audio-view-ctrol-panel`;
        ...
        this.resizable = new Resizable({
            ...
            preserveRatio: true,
            container: view.dom,
            onMove: (_: any, resizeable: Resizable) => {
                this.audio.style.width = `${resizeable.endBox.width}px`;
                if (resizeable && resizeable.el) {
                    resizeable.el.style.height = '100%';
                }
                // this.audio.style.height = `${resizeable.endBox.height}px``;
                this.ctrolPanel.style.pointerEvents = 'auto';
            },
            onEnd: (_: any, resizeable: Resizable) => {
                if (resizeable && resizeable.el) {
                    resizeable.el.style.height = '100%';
                }
    ...
    commitSize(width: number, _: number) {
    ...
    updateStatus() {
        if (!this.audio || !this.node) {
            return;
        }
        this.audio.src = this.node.attrs.src || 'http://music.163.com/song/media/outer/url?id=1449678888.mp3';
        this.audio.title = this.node.attrs.title || '';
        this.audio.controls = true;
        this.audio.preload = 'metadata';
        this.audio.style.width = this.node.attrs.width ? `${this.node.attrs.width}px` : '';
        // this.audio.style.height = this.node.attrs.height ? `${this.node.attrs.height}px` : '';
