...
export const bookmarkPluginKey = new PluginKey('bookmark');
    ...
    const plugin: Plugin = new Plugin({
                ...
                update(view: EditorView) {
                    ...
                    const params: any = {
                        ...
                        command: (url: string) => {
                            ...
                            const bookmarkPluginState = bookmarkPluginKey.getState(state);
                            const currentPos = bookmarkPluginState?.pos;
                            ...
                            // 2. 執行替換/插入
                            const node = state.schema.nodes.bookmark.create({ src: url });
                            const tr = state.tr
                                .replaceWith(currentPos, currentPos, node) // 在精確位置插入
                                .setMeta('bookmark', false);                  // 關閉插件狀態（隱藏模擬光標和彈窗）
        ...
        state: {
            ...
            apply(tr: Transaction, prevValue: any) {
                const meta = tr.getMeta('bookmark');
                if (meta === false) {
                    return {
                        pos: null
                    };
        ...
        props: {
            nodeViews: {
                bookmark: (node: Node, view: EditorView, getPos: () => number) => new BookmarkView(node, view, getPos)
            } as any,
            handleKeyDown(view: EditorView, event: any) {
                const state = bookmarkPluginKey.getState(view.state);
                if (typeof state.pos !== 'number') {
                    return false; // 我不活跃，放行给后面的插件
                }
                if (event.key === 'Escape' || event.key === 'Esc') {
                    view.dispatch(view.state.tr.setMeta('bookmark', false));
                    return true;
                }
                return false;
            },
            decorations(state) {
                ...
                cursor.className = `${CLASSNAME}-bookmark-cursor`;
                return DecorationSet.create(state.doc, [
                    Decoration.widget(pos, cursor, { key: 'bookmark-cursor', side: -1 })
                ]);
            }
        }
    });
