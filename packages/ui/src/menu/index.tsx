...
export const Menu: React.FC<MenuProps> = props => {
    ...
    // activeKeyRef.current = activeKey || (mergeRef.current.length > 0 && mergeRef.current[0].key);
    ...
    useEffect(() => {
        if ('selectedKeys' in props) {
            ...
            // 1. 收集所有選中節點的父級 Key
            // 2. 獲取所有選中節點的父級 Key（扁平化並去重）
            const allParentKeys = Array.from(
                new Set((props.selectedKeys || []).flatMap(key => findParentKeys(props.items, key) || []))
            );
            ...
            // 3. 計算最終合併後的 keys (當前已展開的 + 新選中項的父級)
            const nextOpenKeys = Array.from(new Set([...openKeys, ...allParentKeys]));
            ...
            // 5. 不論是否受控，都通知外部展開狀態發生了變化
            props.onOpenChange?.(nextOpenKeys);
        }
    }, [props.selectedKeys, props.items]);
    ...
    // useEffect(() => {
    //     if (!('activeKey' in props)) {
    //         let newActiveKey = mergeRef.current.length > 0 && mergeRef.current[0].key;
    ...
    const getOpenKeysByChange = (key: string, action: any) => {
        ...
        if (props.mode === 'popuover') {
            ...
            newOpenKeys = (openKeysRef.current || []).slice();
            if (action === 'cancel') {
                newOpenKeys = newOpenKeys.filter((node: any) => key !== node);
            } else if (action === 'active') {
