...
export class Editor extends EventEmitter {
    ...
    constructor(options?: EditorOptions) {
        ...
        if (true) {
            ...
            const serverUrl = `${contextPath}/docWs?userId=${new Date().getTime()}&roomId=200`;
            // const provider = new WebsocketProvider(serverUrl, '100', this.ydoc, {
            //     // resyncInterval: 2000
            // });
            const provider = new WebsocketProvider(serverUrl, '200', ydoc);
    ...
    createSchema() {
        let schema: any = new Schema(schemaBase);
        schema = new Schema({
            nodes: schema.spec.nodes.append(tableNodes({
                tableGroup: 'block',
                cellContent: 'block+', // 允许单元格包含段落、列表等块级内容
            } as any)),
