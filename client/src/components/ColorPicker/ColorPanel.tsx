...
class ColorPanel extends Component<ColorPanelProps, ColorPanelState> {
    ...
    renderHeader() {
        ...
        const { type } = this.state;
        return (
            <div className={`${sprefix}-colorPanel-tabs`}>
                {
                    COLORTYPES.map((item: any, index: number) => {
                        const cls = [`${sprefix}-colorPanel-tab`];
                        if (type === item.name) {
                            cls.push(`${sprefix}-colorPanel-tab-active`);
                        }
                        return (
                            <div 
                                key={ index }
    ...
    renderGlobalColors() {
        ...
        return colors?.map((item: any, index: number) => {
            return (
                <div 
                    key={ index }
                    className={ `${ sprefix }-colorPanel-colors-item` }
                    onClick={ this.handleColorClick(item) }
                >
                    <div 
                        className={ `${ sprefix }-colorPanel-colors-color` }
    ...
    renderInput(type: string, label: string) {
        ...
        const state: any = this.state;
        return (
            <div className={`${sprefix}-colorPanel-rgba`}>
                <input
                    className={`${sprefix}-colorPanel-input`}
                    type="text"
                    value={state[type]}
                ...
                />
                <div className={`${sprefix}-colorPanel-label`}>{label}</div>
            </div>
        );
    }
    renderGradientBar(gradient: Array<any> = []) {
        ...
        return gradient.map((item: any, index: number) => {
            const cls: Array<string> = [`${ sprefix }-colorPanel-gradient-slider-drag`];
            if (this.drag[type] === index) {
                cls.push(`${ sprefix }-colorPanel-gradient-slider-drag-selected`);
            }
            return (
                <div
                    key={ index }
                ...
                >
                    <div className={ `${ sprefix }-colorPanel-gradient-slider-pointer` }></div>
                    <div className={ `${ sprefix }-colorPanel-gradient-slider-body` }>
                        <div
                            className={ `${ sprefix }-colorPanel-gradient-slider-body-inner` }
                            style={{
    ...
    render() {
        const { className, sprefix } = this.props;
        const { type, hue, rgba, pos, hexStr, angle } = this.state;
        const cls = [
            `${sprefix}-colorPanel`,
            // `${sprefix}-${type}-colorPanel`
        ];
        if (className) {
            cls.push(className);
        }
        ...
        return (
            <div className={cls.join(' ')}>
                <div className={`${sprefix}-colorPanel-header`}>
                    {this.renderHeader()}
                </div>
                <div className={`${sprefix}-colorPanel-center`}>
                    <div className={`${sprefix}-colorPanel-gradient ${sprefix}-colorPanel-${type}`}>
                        <div className={`${sprefix}-colorPanel-gradient-bar-bg`}>
                            <div
                                className={`${sprefix}-colorPanel-gradient-bar`}
                                style={{ background: gradientBarBg }}
                            ></div>
                        </div>
                        <div
                            className={`${sprefix}-colorPanel-gradient-slider-bar`}
                            ref={(ref) => this.gradientSliderBar = ref}
                            onMouseDown={this.handleAddDrag}
                        >
                            { gradientBar }
                        </div>
                        <div className={`${sprefix}-colorPanel-gradient-angle-wrap`}>
                            <input
                                className={`${sprefix}-colorPanel-range ${sprefix}-colorPanel-gradient-angle`}
                                type="range"
                                min="0"
                                max="360"
                                title={ angle }
                            ...
                            />
                            <span className={`${sprefix}-colorPanel-gradient-angle-label`}>{ angle }</span>
                            <div
                                className={`${sprefix}-colorPanel-gradient-angle-list`}
                                style={{ display: 'none' }}
                            >
                                <span className={`${sprefix}-colorPanel-gradient-angle-item`}>0</span>
                                <span className={`${sprefix}-colorPanel-gradient-angle-item`}>
                                    <input className={`${sprefix}-colorPanel-input ${sprefix}-colorPanel-gradient-angle-input`} type="text" />
                                </span>
                                <span className={`${sprefix}-colorPanel-gradient-angle-item`}>360</span>
                            </div>
                        </div>
                    </div>
                    <div className={`${sprefix}-colorPanel-center-inner`}>
                        <div
                            className={`${sprefix}-colorPanel-panel`}
                            ref={(ref) => this.panel = ref}
                            onMouseDown={this.handleMouseDown}
                            onTouchStart={this.handleMouseDown}
                        >
                            <div
                                className={`${sprefix}-colorPanel-hue`}
                                style={{ backgroundColor: `rgb(${hueColor.r},${hueColor.g},${hueColor.b})` }}
                            >
                                <div className={`${sprefix}-colorPanel-saturation`}>
                                    <div className={`${sprefix}-colorPanel-brightness`}></div>
                                </div>
                            </div>
                            <div
                                className={`${sprefix}-colorPanel-pointer`}
                                style={{ left: `${pos.x}px`, top: `${pos.y}px` }}
                                // ref={(ref) => this.pointer = ref}
                            ></div>
                        </div>
                        <div className={`${sprefix}-colorPanel-fixed`}>
                            <div className={`${sprefix}-colorPanel-left`}>
                                <div className={`${sprefix}-colorPanel-area-slider-bg`}>
                                    <input
                                        className={`${sprefix}-colorPanel-range ${sprefix}-colorPanel-area-slider`}
                                        type="range"
                                        min="0"
                                        max="360"
                                        value={hue}
                                ...
                                <div className={`${sprefix}-colorPanel-alpha-slider-bg`}>
                                    <input
                                        className={`${sprefix}-colorPanel-range ${sprefix}-colorPanel-alpha-slider`}
                                        type="range"
                                        min="0"
                                        max="1"
                                        step="0.01"
                            ...
                            <div className={`${sprefix}-colorPanel-right`}>
                                <div className={`${sprefix}-colorPanel-preview-bg`}>
                                    <div
                                        className={`${sprefix}-colorPanel-preview`}
                                        style={{ background: prviewBg }}
                                    ></div>
                                    {/* <div className={ `${ sprefix }-colorPanel-gradient-preview` } style={{ background: prviewBg }}></div> */}
                                </div>
                            </div>
                        </div>
                        <div className={`${sprefix}-colorPanel-color-panel`} >
                            <div className={`${sprefix}-colorPanel-hex-wrap`} >
                                <span className={`${sprefix}-colorPanel-hex-label`} >#</span>
                                <input
                                    className={`${sprefix}-colorPanel-input`}
                                    value={hexStr}
                                    maxLength={6}
                                ...
                                />
                                <div className={`${sprefix}-colorPanel-label`} >HEX</div>
                            </div>
                            <div className={`${sprefix}-colorPanel-rgba-wrap`} >
                ...
                <div className={`${sprefix}-colorPanel-footer`}>
                    <div className={`${sprefix}-colorPanel-global`}>
                        <input className={`${sprefix}-colorPanel-chk`} defaultChecked type="checkbox" />
                        <div className={`${sprefix}-colorPanel-arrow-wrap`}>
                            <span className={`${sprefix}-colorPanel-arrow`}>
                            ...
                            <span className={`${sprefix}-colorPanel-nav-label`}>Global Colors</span>
                        </div>
                        <div className={`${sprefix}-colorPanel-list ${sprefix}-colorPanel-colors`}>
                            {this.renderGlobalColors()}
                        </div>
                    </div>
                    {/* <div className={`${sprefix}-colorPanel-recent`}>
                        <input className={`${sprefix}-colorPanel-chk`} type="checkbox" />
                        <div className={`${sprefix}-colorPanel-arrow-wrap`}>
                            <span className={`${sprefix}-colorPanel-arrow`}>
                                <svg viewBox="0 0 1024 1024" focusable="false" data-icon="caret-down" width="1em" height="1em" fill="currentColor" aria-hidden="true">
                                    <path d="M840.4 300H183.6c-19.7 0-30.7 20.8-18.5 35l328.4 380.8c9.4 10.9 27.5 10.9 37 0L858.9 335c12.2-14.2 1.2-35-18.5-35z"></path>
                            ...
                            <span className={`${sprefix}-colorPanel-nav-label`}>Recent Colors</span>
                        </div>
                        <div className={`${sprefix}-colorPanel-list`}></div>
                    </div> */}
                </div>
                <input className={`${sprefix}-colorPanel-hidden`} type="hidden" />
            </div>
        );
    }
}
