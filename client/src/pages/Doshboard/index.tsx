...
const Page: React.FC<IProps> = props => {
    ...
    const [fullDataList, setFullDataList] = useState<Array<any>>([]);
    ...
    const getListStock = async (params?: any) => {
        setLoading(true);
        const result = await listStockDoshboard(params);
        setLoading(false);
        if (result.code === 200) {
            const list = result.data || [];
            const { start, end } = startEnd(pagination.pageNum, pagination.pageSize);
            setDataList(list.slice(start, end));
            setFullDataList(list);
            setPagination({
                pageNum: pagination.pageNum,
                pageSize: pagination.pageSize,
                total: list.length
    ...
    const handlePagination = ({
        ...
        const { start, end } = startEnd(pageNum, pageSize);
        setDataList(fullDataList.slice(start, end));
        // getListStock({
        //     ...queryParams,
        // 	pageNum,
        // 	pageSize
